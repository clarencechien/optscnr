...
_README 由 build_readme.py 於 2026-09-15 08:37 UTC 重組；兩區塊各為該雷達最近一次排程的輸出，時間戳以區塊內為準。_
...
- 鋒面 ☀️ RISK_ON（本週未變）。
- 2308 前提 🟡 YELLOW；無背離（前提 YELLOW、價格 20 日 -8.2%）；PER 51.6，3 年分位 83 → 74
- 2330 前提 🟡 YELLOW；無背離（前提 YELLOW、價格 20 日 +0.2%）；PER 27.6，3 年分位 71 → 68
- 投降窗加碼歷史 8 次：比等到例行日買平均便宜 +1.17%、勝率 75%；平均成本 vs 純 DCA +0.71%。

## 1. 本期機械指示（DCA 規則影子帳本）

- **週檢查・月預算**（決策日 2026-09-18）：本月預算已於 2026-09-14 動用（月末例行），本月不再行動
...
|---|---|---|---|---|---|
| 純定期定額 | 93 | 32.61 | +0.00% | +227.84% | +30.22% |
| 週檢查・月預算・投降窗即刻投入 | 93 | 33.06 | +1.38% | +223.33% | +30.65% |
| 定期定額＋投降窗加碼 | 101 | 32.84 | +0.71% | +225.50% | +30.37% |
| 定期定額＋鋒面調節 | 93 | 32.79 | +0.55% | +226.05% | +30.16% |
| 對照：一次投入 | 1 | 18.5 | -43.27% | +477.73% | +25.62% |

投降窗加碼 8 次：edge +1.17%、勝率 75%。 明細見 `dca_ledger.md`。

## 2. 天氣（籌碼溫度計）

- 鋒面 ☀️ **RISK_ON**（score +0.25，2026-09-14）；本週未變
- 分位數：外資現貨 63.1、大台Δ 85.3、散戶小台 12.7、融資Δ 14.3
- 本週投降警報：無
...
- 無背離（前提 YELLOW、價格 20 日 -8.2%）；PER 51.6，3 年分位 83 → 74
  ...
  - 🟢 M5 narrative_triggers：capex_cut:4e / vr300_delay:18e(20m) / debt_financed_capex:11e(12m) / lc_psu_competition:0e
  - 🔴 M6 peer_divergence：cooling:3324領先+36pp
  - ⚪ M8 revision_velocity：下修 0/上修 0（樣本不足 <3，NO_DATA）
  - 🟢 M9 valuation：PER 51.6（3 年第 74 百分位；20 日前第 83）；觀察
- 回填樣本 83 筆（T+20 超額）；退役判準見 `delta_radar_report.md`

### 2330
- 前提（最近全模組 2026-09-14）：🟡 YELLOW
- 無背離（前提 YELLOW、價格 20 日 +0.2%）；PER 27.6，3 年分位 71 → 68
  ...
  - 🔴 M5 narrative_triggers：export_controls_tariffs:16e / n2_arizona_ramp:9e(10m) / cowos_capacity:6e / geopolitics:5e / hyperscaler_capex:4e
...
|---|---|---|---|---|---|---|
| 3661 | 世芯-KY | ASIC | -12.00% | +273.60% | +156.90% | +129.11 |
| 2382 | 廣達 | 機櫃 | -0.72% | +177.50% | +137.20% | +37.29 |
| 2383 | 台光電 | CCL | -16.25% | +129.80% | +126.60% | +4.58 |
| 3443 | 創意 | ASIC | +8.98% | +111.00% | +124.40% | +3.65 |
| 3231 | 緯創 | 機櫃 | -3.63% | +166.50% | +93.70% | +56.32 |
| 3324 | 雙鴻 | 散熱 | +29.43% | +67.20% | +82.00% | +2.52 |
| 2345 | 智邦 | 網通 | -18.54% | +59.40% | +64.10% | -0.87 |
| 3017 | 奇鋐 | 散熱 | +4.66% | +54.30% | +59.30% | -5.89 |
| 2330 | 台積電 | 製造 | -1.25% | +53.30% | +55.30% | -7.27 |
| 2317 | 鴻海 | 機櫃 | -2.97% | +52.00% | +52.80% | -0.07 |
| 2308 | 台達電 | 電力 | -14.48% | +34.90% | +46.00% | -10.24 |
| 3037 | 欣興 | 載板 | -13.81% | +56.30% | +45.40% | +9.97 |
| 3711 | 日月光投控 | 封測 | -0.91% | +45.70% | +40.60% | +6.40 |
| 6669 | 緯穎 | 機櫃 | +2.45% | +50.40% | +39.80% | +10.28 |
| 2301 | 光寶科 | 電源 | +4.89% | +25.20% | +33.30% | -5.89 |
| 2454 | 聯發科 | IC設計 | +12.17% | +44.10% | +19.70% | +20.64 |

影子 DCA（每月等權買整籃 vs 同筆錢買 0050）：33 個月，籃子 +192.78% vs 0050 +103.51%，差 +89.26 pp；判準 可讀
月營收加速三分位 vs T+20 超額：累積中（回填 0 筆）。明細 `casino_report.md`。
*賭場 sector：小部位、預算固定、名單是人挑的、沒有訊號、期望值未證明。只收資料。*

## 5. 下週日曆

- 2026-10-10（25 天後）台股月營收公告截止（2330／2308 通常 10 日前後）

## 6. 資料健康

- ✅ 天氣台：最新 2026-09-14
- ✅ DCA 帳本：最新 2026-09-14
- ✅ delta_radar：最新 2026-09-15
- ✅ tsmc_radar：最新 2026-09-15
- ✅ 賭場 sector：最新 2026-09-14

---
*tw_brief — 週報：規則影子帳本＋溫度計＋2308／2330 論點監控＋賭場 sector。沒有任何一行是買賣建議；曝險與部位由人管。 產出 2026-09-15T08:37:06+00:00*
...
# 💰 DCA 規則影子帳本（0050）— 至 2026-09-14

每月 6 日後首個交易日買 10,000 元；價格 2019-01-02 起、鋒面/警報序列 2019-01-02 起（1870 日）。等額假設、不記真實部位。
...
## 視窗：full（2019-01-02 → 2026-09-14，93 期）

| 規則 | 買次 | 投入 | 單位 | 平均成本 | vs 純 DCA | 期末市值 | 報酬 | XIRR |
|---|---|---|---|---|---|---|---|---|
| 純定期定額 | 93 | 930,000 | 28,521.02 | 32.61 | +0.00% | 3,048,897 | +227.84% | +30.22% |
| 週檢查・月預算・投降窗即刻投入 | 93 | 990,000 | 29,943.49 | 33.06 | +1.38% | 3,200,959 | +223.33% | +30.65% |
| 定期定額＋投降窗加碼 | 101 | 1,010,000 | 30,753.05 | 32.84 | +0.71% | 3,287,501 | +225.50% | +30.37% |
| 定期定額＋鋒面調節 | 93 | 1,015,000 | 30,958.32 | 32.79 | +0.55% | 3,309,445 | +226.05% | +30.16% |
| 對照：一次投入 | 1 | 930,000 | 50,261.25 | 18.50 | -43.27% | 5,372,928 | +477.73% | +25.62% |
...
## 視窗：last_12m（2025-09-15 → 2026-09-14，12 期）

| 規則 | 買次 | 投入 | 單位 | 平均成本 | vs 純 DCA | 期末市值 | 報酬 | XIRR |
|---|---|---|---|---|---|---|---|---|
| 純定期定額 | 12 | 120,000 | 1,503.48 | 79.81 | +0.00% | 160,722 | +33.93% | +78.62% |
| 週檢查・月預算・投降窗即刻投入 | 13 | 150,000 | 1,899.22 | 78.98 | -1.04% | 203,027 | +35.35% | +82.08% |
| 定期定額＋投降窗加碼 | 14 | 140,000 | 1,707.82 | 81.98 | +2.72% | 182,566 | +30.40% | +78.97% |
| 定期定額＋鋒面調節 | 12 | 140,000 | 1,724.78 | 81.17 | +1.70% | 184,379 | +31.70% | +77.90% |
| 對照：一次投入 | 1 | 120,000 | 1,943.23 | 61.75 | -22.63% | 207,731 | +73.11% | +79.62% |
