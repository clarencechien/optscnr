...
### 電子報版面：上半白話版、下半可收合完整資料（手機優先）

`/brief` 上半是「一分鐘白話版」：第一行是今天／本週要不要動，接著三到四句白話（天氣、2308 前提、賭場籃子、資料狀態）。
下半是完整資料，每段是 `<details>`：手機預設收合、桌機（≥900px）預設展開；表格包在 `.scroll` 裡橫向捲動，頁面本身不會橫向捲。

### 電子報的市場切換：`/brief?m=tw` 只呈現台股

同一頁 `/brief` 靠參數切換：沒帶或 `?m=us` 是美股湯姆熊電子報；**`?m=tw` 只呈現台股週報**
（週檢查・月預算的機械指示、DCA 規則影子帳本、籌碼溫度計、2308 論點監控與背離旗標、下週日曆、資料健康），
頁首有 US／TW 切換鍵。台股資料來自 `/api/tw`（Worker 從 GitHub 讀 `tw_scanner/output/tw_brief.json`，
快取 5 分鐘；`/api/brief?m=tw` 也會轉到同一份），**不觸發任何動作**。JSON 由 Actions 的
`tw_scanner/tw_brief.py` 在天氣台／delta_radar 跑完後組裝；Worker 只轉發。舊路徑 `/tw` 轉址到 `/brief?m=tw`。
要公開分享，Zero Trust 的 Bypass 應用程式再加兩條 path：`/tw` 與 `/api/tw`（`/brief` 已涵蓋帶參數的情況）。
...
| 任何時間 | `/api/health` 排程健康；`/api/alarm/check` 手動補發；`POST /api/decide` 手動產生 decisions；`/api/decisions` 最近 N 天；`/api/facts` 解析後的事實庫；**`/brief` + `/api/brief` 公開唯讀電子報**；**`/brief?m=tw` + `/api/tw` 公開唯讀台股週報** |
