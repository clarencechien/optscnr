...
你的工作只有一題事實題（排定事件）＋事實庫提案，不做「可玩／跳過」的最終判斷，不填機率，不編勝率。
近 5 日跳空與 Δ7d 由程式直接計算，不必回答，也不要寫進 note。
...
- 每一張輸入的候選都要有「剛好一筆」輸出（q_event 與 note 放在同一個物件），signal_id 原樣照抄，不得增刪、不得拆成兩筆。
...
| `candidates[].q_event, note` | LLM 答案（答不到的 candidate → `status: "unanswered"`） |
| `candidates[].q_gap, q_delta` | **Worker 計算**（v4.2 起；`method: "worker"`）：跳空用 `recent_spots` 對 `data/universe_spots` 交易日曆算單日變動，Δ7d 照 `oi_d7`／`oi_delta_status`。LLM 失敗時照樣有值 |
...
| v4.2 | 2026-09-23 | (b) 跳空、(c) Δ7d 改 Worker 確定性計算，LLM 只答 (a)；要求每張候選剛好一筆（9/11 曾拆成兩筆導致事件答案遺失）。歷史回放 8 張可比者 (b)(c) 與 LLM 答案全一致 |
